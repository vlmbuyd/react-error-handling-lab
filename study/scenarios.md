...
### 발표 후 받은 질문(2026-06-19) — 위 3개의 심화 probe

> 기본개념 학습이 끝나면 이 셋에 *내 말로* 답할 수 있어야 한다. 답은 적지 않는다.

- **A (→1 심화).** ErrorBoundary는 왜 하필 class인가? 콜스택 문제만 푸는 거라면 전역 컨텍스트 같은 다른 설계도 가능했을 텐데, 왜 트리 위치를 가진 class 컴포넌트였나? *(probe: 전역 컨텍스트면 무엇을 잃나? — 어느 트리가 망가졌고 어디에 fallback을 그릴지)*
- **B (→3 정문).** 자식의 에러를 부모가 떠안는 건 하향식 데이터 흐름과 반대되는 IOC처럼 보인다. 이게 왜 "더 선언적"인가? *(probe: try/catch의 throw도 콜스택 위로 간다. React가 다른 건 '전파'가 아니라 그걸 트리 위치로 선언했다는 점)*
- **C (→2·3).** 한 컴포넌트에서 여러 종류의 Promise를 던지면 Suspense가 이를 어떻게 구분하나? *(probe: 전제를 의심하라 — Suspense가 정말 '구분'하나? 여러 데이터의 정체성은 어느 층이 책임지나?)*
