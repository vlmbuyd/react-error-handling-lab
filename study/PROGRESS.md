...
- **현재 버전**: v0 (진행 중 — 시나리오 1 완료, 06-11에 콘솔 예제로 복습/재확인)
- **마지막 세션 날짜**: 2026-06-11
...
| # | 질문 | 상태 |
|---|------|------|
| 1 | try/catch의 한계 + ErrorBoundary 메커니즘 + 왜 아직 class | 🟡 (비동기 throw를 try/catch가 못 잡는 이유 + `.catch`와의 채널 차이는 콘솔 예제로 재확인까지 끝나 🟢 수준으로 설명 가능. 렌더/이벤트 에러 한계, ErrorBoundary는 아직) |
...
- [~] v0 — async rejection을 try/catch가 못 잡는 건 직접 확인 ✅ + 콜스택/마이크로태스크로 *왜*까지 콘솔 예제로 재확인 ✅ / 렌더·이벤트 에러는 다음 세션
